...
class SmartRoadApp:
        ...
        if uploaded_file is not None:
            ...
            with col_result:
                ...
                if st.session_state.processed_file == uploaded_file.name:
                    ...
                    m1.metric("AREA", f"{metrics['crack_area_px']:,}")
                    m2.metric("RATIO", f"{metrics['crack_ratio_pct']:.1f}%")
                    m3.metric("LENGTH", f"{metrics['crack_length_px']:.0f}px")
                    m4.metric("DENSITY", f"{metrics['crack_density']:.0f}")

                    m5, m6, m7 = st.columns(3)
                    m5.metric("SEGMENTS", metrics.get("num_crack_segments", "-"))
                    m6.metric("MAX WIDTH", f"{metrics.get('max_crack_width_px', 0):.0f}px")
                    m7.metric("SEVERITY", f"{metrics.get('severity_score', 0):.0f}/100")

                    st.progress(min(int(metrics.get("severity_score", 0)), 100))
    ...
    def halaman_dashboard(self):
        ...
        with c2:
            ...
            st.dataframe(
                df[['timestamp', 'filename', 'crack_ratio_pct', 'severity_score', 'condition']].sort_values("timestamp", ascending=False),
